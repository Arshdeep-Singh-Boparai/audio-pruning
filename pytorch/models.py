...
class Cnn14_4channel(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np
        
        super(Cnn14_4channel, self).__init__()
        ...
        path = '/mnt/fast/nobackup/users/as0150/arshdeep/arshdeep/PANNS_pruning/sorted_index/OP' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'   
        ...
        self.conv_block1 = ConvBlock_pruned(in_channels_1=4, out_channels_1=int(64*(1-p1)),out_channels_2=int(64*(1-p2)))
        ...
        checkpoint_path = '/mnt/fast/nobackup/scratch4weeks/as0150/panns_transfer_to_gtzan-master/pretrained_checkpoint/Cnn14_mAP=0.431.pth'
        ...
        #self.load_state_dict(weights_pruned)   #load pre-trained weights here
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x_quaternion = torch.cat([x,x_first, x_second, x_third], dim=1) 
 
 
        x = self.logmel_extractor(x_quaternion)    # (batch_size, 1, time_steps, mel_bins)
...
class Cnn14_4channel_1(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np
        
        super(Cnn14_4channel_1, self).__init__()
...
class Cnn14_pruned_v7_OP(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np
        
        super(Cnn14_pruned_v7_OP, self).__init__()
...
class Cnn14_pruned_CS_graph_sim(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_CS_graph_sim, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/graph_sim/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'
        ...
        C1 = sorted(np.load(os.path.join(path,'conv_block1.conv1.weight.npy'))[64-43:64])
        C2= sorted(np.load(os.path.join(path,'conv_block1.conv2.weight.npy'))[64-51:64])


        C3 = sorted(np.load(os.path.join(path,'conv_block2.conv1.weight.npy'))[128-100:128])
        C4 = sorted(np.load(os.path.join(path,'conv_block2.conv2.weight.npy'))[128-98:128])


        C5 = sorted(np.load(os.path.join(path,'conv_block3.conv1.weight.npy'))[256-198:256])
        C6 = sorted(np.load(os.path.join(path,'conv_block3.conv2.weight.npy'))[256-201:256])


        C7 = sorted(np.load(os.path.join(path,'conv_block4.conv1.weight.npy'))[512-418:512])
        C8 = sorted(np.load(os.path.join(path,'conv_block4.conv2.weight.npy'))[512-412:512])


        C9 = sorted(np.load(os.path.join(path,'conv_block5.conv1.weight.npy'))[1024-816:1024])
        C10 = sorted(np.load(os.path.join(path,'conv_block5.conv2.weight.npy'))[1024-842:1024])


        C11 = sorted(np.load(os.path.join(path,'conv_block6.conv1.weight.npy'))[2048-1658:2048])
        C12 = sorted(np.load(os.path.join(path,'conv_block6.conv2.weight.npy'))[2048-1640:2048])
        ...
        self.conv_block1 = ConvBlock_pruned(in_channels_1=1, out_channels_1=int(len(C1)),out_channels_2=int(len(C2)))
        self.conv_block2 = ConvBlock_pruned(in_channels_1=int(len(C2)), out_channels_1=int(len(C3)),out_channels_2=int(len(C4)))
        self.conv_block3 = ConvBlock_pruned(in_channels_1=int(len(C4)), out_channels_1=int(len(C5)),out_channels_2=int(len(C6)))
        self.conv_block4 = ConvBlock_pruned(in_channels_1=int(len(C6)), out_channels_1=int(len(C7)),out_channels_2=int(len(C8)))
        self.conv_block5 = ConvBlock_pruned(in_channels_1=int(len(C8)), out_channels_1=int(len(C9)),out_channels_2=int(len(C10)))
        self.conv_block6 = ConvBlock_pruned(in_channels_1=int(len(C10)), out_channels_1=int(len(C11)),out_channels_2=int(len(C12)))

        self.fc1 = nn.Linear(int(len(C12)), 2048, bias=True)
        self.fc_audioset = nn.Linear(2048, classes_num, bias=True)
...
class Cnn14_pruned_v5_graph_sim_weighted_degree_25(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_graph_sim_weighted_degree_25, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/graph_sim_weighted_degree/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'
...
class Cnn14_pruned_v5_graph_sim_25(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_graph_sim_25, self).__init__()
...
class Cnn14_pruned_v5_graph_sim_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_graph_sim_50, self).__init__()
...
class Cnn14_pruned_v5_graph_sim_90(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_graph_sim_90, self).__init__()
...
class Cnn14_pruned_v5_CS(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_CS, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/CS/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'   
...
class Cnn14_pruned_v5_OP(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np
        
        super(Cnn14_pruned_v5_OP, self).__init__()
...
class Cnn14_pruned_v5_GM_random_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_GM_random_50, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/GM/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'   
...
class Cnn14_pruned_v5_L1_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np
        
        super(Cnn14_pruned_v5_L1_50, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/L1/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'   
...
class Cnn14_pruned_v5_L1_90(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_L1_90, self).__init__()
        ...
        C1 = sorted(np.load(os.path.join(path,'conv_block1.conv1.weight.npy'))[int(np.ceil(64*p1)):64])
        C2= sorted(np.load(os.path.join(path,'conv_block1.conv2.weight.npy'))[int(np.ceil(64*p2)):64])


        C3 = sorted(np.load(os.path.join(path,'conv_block2.conv1.weight.npy'))[int(np.ceil(128*p3)):128])
        C4 = sorted(np.load(os.path.join(path,'conv_block2.conv2.weight.npy'))[int(np.ceil(128*p4)):128])


        C5 = sorted(np.load(os.path.join(path,'conv_block3.conv1.weight.npy'))[int(np.ceil(256*p5)):256])
        C6 = sorted(np.load(os.path.join(path,'conv_block3.conv2.weight.npy'))[int(np.ceil(256*p6)):256])


        C7 = sorted(np.load(os.path.join(path,'conv_block4.conv1.weight.npy'))[int(np.ceil(512*p7)):512])
        C8 = sorted(np.load(os.path.join(path,'conv_block4.conv2.weight.npy'))[int(np.ceil(512*p8)):512])


        C9 = sorted(np.load(os.path.join(path,'conv_block5.conv1.weight.npy'))[int(np.ceil(1024*p9)):1024])
        C10 = sorted(np.load(os.path.join(path,'conv_block5.conv2.weight.npy'))[int(np.ceil(1024*p10)):1024])


        C11 = sorted(np.load(os.path.join(path,'conv_block6.conv1.weight.npy'))[int(np.ceil(2048*p11)):2048])
        C12 = sorted(np.load(os.path.join(path,'conv_block6.conv2.weight.npy'))[int(np.ceil(2048*p12)):2048])
        ...
        self.conv_block1 = ConvBlock_pruned(in_channels_1=1, out_channels_1=int(64-np.ceil(64*p1)),out_channels_2=int(64-np.ceil(64*p2)))
        self.conv_block2 = ConvBlock_pruned(in_channels_1=int(64-np.ceil(64*p2)), out_channels_1=int(128-np.ceil(128*p3)),out_channels_2=int(128-np.ceil(128*p4)))
        self.conv_block3 = ConvBlock_pruned(in_channels_1=int(128-np.ceil(128*p4)), out_channels_1=int(256-np.ceil(256*p5)),out_channels_2=int(256-np.ceil(256*p6)))
        self.conv_block4 = ConvBlock_pruned(in_channels_1=int(256-np.ceil(256*p6)), out_channels_1=int(512-np.ceil(512*p7)),out_channels_2=int(512-np.ceil(512*p8)))
        self.conv_block5 = ConvBlock_pruned(in_channels_1=int(512-np.ceil(512*p8)), out_channels_1=int(1024-np.ceil(1024*p9)),out_channels_2=int(1024-np.ceil(1024*p10)))
        self.conv_block6 = ConvBlock_pruned(in_channels_1=int(1024-np.ceil(1024*p10)), out_channels_1=int(2048-np.ceil(2048*p11)),out_channels_2=int(2048-np.ceil(2048*p12)))

        self.fc1 = nn.Linear(int(2048-np.ceil(2048*p12)), 2048, bias=True)
        self.fc_audioset = nn.Linear(2048, classes_num, bias=True)
...
class ResNet38_4channel(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        
        super(ResNet38_4channel, self).__init__()
...
class Cnn14_pruned_v5_HRank_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_HRank_50, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/HRank/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'
...
class Cnn14_pruned_v5_Energy_aware_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_Energy_aware_50, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/Energy_aware/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'
...
class Cnn14_pruned_v5_entropy_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin,
        fmax, classes_num, pooling_type, pooling_factor):
        import os
        import numpy as np

        super(Cnn14_pruned_v5_entropy_50, self).__init__()
        ...
        top_db = None
        from collections import OrderedDict
        path = '/mnt/fast/nobackup/scratch4weeks/as0150/arshdeep/PANNs_pruning/sorted_Indexes/entropy/' #'/home/arshdeep/Pruning/PANNs_pruning/sorted_Indexes/OP/'
...
def after_pruning_bn_parameters(unpruned_weights, sorted_index, pruning_ratio, layer_name):
    ...
    Q1 = Q[0:int(len_Q)][sorted_index[0:int(len(sorted_index)*(1-pruning_ratio))]]
...
def fully_connected_layers_pruning(unpruned_weights, sorted_index_conv2, pruning_ratio, layer_name):
    Q = unpruned_weights[layer_name].numpy()
    len_Q = len(Q)
    Q1 = Q[:, sorted_index_conv2[0:int(len(sorted_index_conv2)*(1-pruning_ratio))]]
    after_pruning_tensor = torch.tensor(Q1)    
    
    return after_pruning_tensor


class ResNet38_pruned_25(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        
        super(ResNet38_pruned_25, self).__init__()
        ...
        self.conv_block_after1 = ConvBlock(in_channels=512, out_channels=int(2048*(1-p)))

        self.fc1 = nn.Linear(int(2048*(1-p)), 2048)
        self.fc_audioset = nn.Linear(2048, classes_num, bias=True)

        # self.init_weights()
        checkpoint_path = '/mnt/fast/nobackup/users/as0150/arshdeep/arshdeep/ResNet38/checkpoint_resnet38/ResNet38_mAP=0.434.pth'
        ...
        w_resnet_unpruned  = weights

        w_resnet_pruned = weights
        # user input 
        conv_block_after1_conv1_important_index = np.load('/mnt/fast/nobackup/users/as0150/arshdeep/arshdeep/ResNet38/sorted_index_ResNet38/GM/conv_block_after1.conv1.weight.npy')
        conv_block_after1_conv2_important_index = np.load('/mnt/fast/nobackup/users/as0150/arshdeep/arshdeep/ResNet38/sorted_index_ResNet38/GM/conv_block_after1.conv2.weight.npy')


        sorted_index_bottleneck = np.arange(0,int(512))

        sorted_index_conv1 =  conv_block_after1_conv1_important_index
        sorted_index_conv2 =  conv_block_after1_conv2_important_index
        
        
        w_resnet_pruned['conv_block_after1.conv1.weight'] =  after_pruning_parameters(w_resnet_unpruned, sorted_index_conv1, sorted_index_bottleneck, pruning_ratio = p, layer_name = 'conv_block_after1.conv1.weight', prev_layer_pruned = 'no') # (out, input, filter size)
        w_resnet_pruned['conv_block_after1.bn1.running_mean'] = after_pruning_bn_parameters(w_resnet_unpruned, conv_block_after1_conv1_important_index, pruning_ratio = p, layer_name = 'conv_block_after1.bn1.running_mean')
        w_resnet_pruned['conv_block_after1.bn1.running_var'] = after_pruning_bn_parameters(w_resnet_unpruned, conv_block_after1_conv1_important_index, pruning_ratio = p, layer_name = 'conv_block_after1.bn1.running_var')
        w_resnet_pruned['conv_block_after1.bn1.bias'] = after_pruning_bn_parameters(w_resnet_unpruned, conv_block_after1_conv1_important_index, pruning_ratio = p, layer_name = 'conv_block_after1.bn1.bias')
        w_resnet_pruned['conv_block_after1.bn1.weight'] = after_pruning_bn_parameters(w_resnet_unpruned, conv_block_after1_conv1_important_index, pruning_ratio = p, layer_name = 'conv_block_after1.bn1.weight')
        w_resnet_pruned['conv_block_after1.conv2.weight'] =  after_pruning_parameters(w_resnet_unpruned, sorted_index_conv2, sorted_index_conv1, pruning_ratio = p, layer_name = 'conv_block_after1.conv2.weight',prev_layer_pruned = 'yes') # (out, input, filter size)
        w_resnet_pruned['conv_block_after1.bn2.running_mean'] = after_pruning_bn_parameters(w_resnet_unpruned, sorted_index_conv2, pruning_ratio = p, layer_name = 'conv_block_after1.bn2.running_mean')
        w_resnet_pruned['conv_block_after1.bn2.running_var'] = after_pruning_bn_parameters(w_resnet_unpruned, sorted_index_conv2, pruning_ratio = p, layer_name = 'conv_block_after1.bn2.running_var')
        w_resnet_pruned['conv_block_after1.bn2.bias'] = after_pruning_bn_parameters(w_resnet_unpruned, sorted_index_conv2, pruning_ratio = p, layer_name = 'conv_block_after1.bn2.bias')
        w_resnet_pruned['conv_block_after1.bn2.weight'] = after_pruning_bn_parameters(w_resnet_unpruned, sorted_index_conv2, pruning_ratio = p, layer_name = 'conv_block_after1.bn2.weight')
        w_resnet_pruned['fc1.weight']   =  fully_connected_layers_pruning(w_resnet_unpruned, sorted_index_conv2, pruning_ratio = p, layer_name = 'fc1.weight')

        self.load_state_dict(w_resnet_pruned)
...
class ResNet38_pruned_50(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        
        super(ResNet38_pruned_50, self).__init__()
...
class ResNet38_pruned_75(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        
        super(ResNet38_pruned_75, self).__init__()
