...
sys.path.append('/mnt/fast/nobackup/scratch4weeks/as0150/audioset_tagging_cnn')
import numpy as np
import argparse
import time
import logging
...
from hear21passt.base import get_basic_model,get_model_passt   #passt model

# HDF5_PATH = "/mnt/fast/datasets/audio/audioset/waveforms_hdf5s"

# torch.backends.cudnn.enabled = False
os.environ["WANDB_API_KEY"] = "0948c82c21bbd8aed452c5c991ca25ba0b15099d"
wandb.init(
    # set the wandb project where this run will be logged
    project="passt_prune_KD_QCNN", name = 'prune_KD',
...
def train(args):
    ...
    fmin = args.fmin
    fmax = args.fmax
    model_type_teacher = args.model_type_teacher #KD change
    model_type = args.model_type #KD change    
    loss_type = args.loss_type
    loss_type_KD = args.loss_type_KD
    ...
    loss_func_KD = get_loss_func(loss_type_KD)
    ...
    if(model_type == "mobilenet"):
        ...
        #Model_teacher = eval(model_type_teacher)
        Model_student = eval(model_type)
        model = Model_student(sample_rate=sample_rate, window_size=window_size, hop_size=hop_size, mel_bins=mel_bins, fmin=fmin, fmax=fmax, classes_num=classes_num)#, pooling_factor=pooling_factor, pooling_type=pooling_type)
        model_teacher = get_basic_model(model = 'logits')  #Model_teacher(sample_rate=sample_rate, window_size=window_size, hop_size=hop_size, mel_bins=mel_bins, fmin=fmin, fmax=fmax, classes_num=classes_num)#, pooling_factor=pooling_factor, pooling_type=pooling_type) #passt model loading
        #model_teacher = eval(model_teacher)        
    ...
    for batch_data_dict in train_loader:
        ...
        if (iteration % 2000 == 0 and iteration > resume_iteration and iteration > 1000): # or (iteration == 0):
            ...
            wandb.log({"test_mAPs":np.mean(test_statistics['average_precision'])})
        ...
        if 'mixup' in augmentation:
            ...
            with torch.no_grad():
            	batch_data_dict_teacher = torch.sigmoid(model_teacher(batch_data_dict['waveform']))  #output of passt, logits to confidence conversion
        else:
        ...
        # KD loss                
        #loss_KD = loss_KD( batch_output_dict, batch_data_dict_teacher)
        
        # student Loss
        loss = loss_func(batch_output_dict, batch_target_dict) + loss_func_KD(batch_output_dict, batch_data_dict_teacher)
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--model_type_teacher', type=str, required=True)    
    parser_train.add_argument('--loss_type', type=str, default='clip_bce', choices=['clip_bce'])
    parser_train.add_argument('--loss_type_KD', type=str, default='loss_KD', choices=['loss_KD'])
